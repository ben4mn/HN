...
const Stories = {
      ...
      container.appendChild(createElement('div', {
        ...
        innerHTML: `
          <div class="flex gap-2">
            <div class="skeleton bg-gray-200 dark:bg-gray-700 w-10 h-9 mt-0.5 shrink-0 rounded"></div>
  ...
  renderStory(story, rank) {
    ...
    row.innerHTML = `
      <div class="flex gap-2">
        <div class="flex flex-col items-center justify-center shrink-0 w-10 pt-0.5">
          <svg class="w-4 h-4 text-hn mb-0.5" fill="currentColor" viewBox="0 0 12 12"><path d="M6 0l2 4h4l-3 3 1 5-4-3-4 3 1-5L0 4h4z"/></svg>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-3 mt-1.5 text-xs text-gray-500 dark:text-gray-400">
            ...
            <a href="https://news.ycombinator.com/item?id=${story.id}" target="_blank" rel="noopener"
               class="reply-hn-btn flex items-center gap-1.5 px-2.5 py-1 rounded text-hn hover:bg-orange-50 dark:hover:bg-gray-800 transition-colors">
              <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        ${hasUrl ? `<div class="thumbnail-container shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center self-center" data-url="${escapeHtml(story.url)}"></div>` : ''}
        <button class="comment-btn shrink-0 flex flex-col items-center justify-center w-14 rounded-lg text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-hn transition-colors touch-target"
                data-id="${story.id}">
